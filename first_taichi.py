...
n_particles = int(init_particle_size_x *
                  init_particle_size_y * n_grid * n_grid * 9)
...
# limit of volume recording (log space), tuned for stability
volume_recording_limit = 3.0
...
def ProjectDruckerPrager(S: ti.template(), logSp: ti.template()):
    ...
    for d in ti.static(range(2)):
        ...
        S[d, d] = max(
            1e-6, abs(S[d, d] * ti.exp(logSp * volume_recovery_rate)))
    ...
    if S[0, 0] * S[1, 1] >= 1.0:  # Project to tip
        ...
        logSp = min(volume_recording_limit, logSp +
                    ti.log(max(1e-6, JSe)) * 0.5)
...
def Substep():
    ...
    for p in x:
        ...
        if param_flip_vel_adj > 0.0:
            ...
            x[p] = xp + (new_v + flip_pos_adj *
                         param_flip_vel_adj * diff_vel) * dt
        else:
            # apply PIC advection
            v[p] = new_v
            x[p] = xp + new_v * dt
...
def DrawCapsule(gui, radius, half_length, translation, rotation, color):
    ...
    end_pos = np.array(
        [
            [ct[0] + half_length *
                np.cos(phi), ct[1] + half_length * np.sin(phi)],
            [ct[0] - half_length *
                np.cos(phi), ct[1] - half_length * np.sin(phi)],
        ]
    )
    ...
    gui.text(current_advection.name, pos=[
             ct[0] + scheme_label_offset_x, ct[1]], font_size=30)
